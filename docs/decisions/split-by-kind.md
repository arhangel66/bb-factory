...
- [x] `.gitignore`: `state/ .venv/ __pycache__/ .pytest_cache/ .ruff_cache/ .idea/ .DS_Store untracked/`
- [x] `git init`, first signed commit of the tree as it is (`initial factory`)
- [x] every step below is its own commit; tests green before each

## 1. Delete the dead
- [x] `main.py` (a sketch, not valid Python)
- [x] `save_roles()`, `ROLES` in board.py — replaced by step 3
- [x] `untracked/scripts/events_v2.py`, `check_ask_task.py` (superseded by timeline.py and do.py)
- [x] `docs/ideas.md` (the timeline exists), `.idea/` out of git
...
```
- [x] move files with `git mv`, fix imports, `PROMPTS` path, `Handler.directory`, the page's `/dashboard/` URL
- [x] `Config` becomes `dict[Role, AgentConfig]`: `self.config[role]` instead of `getattr`; construct.py builds the dicts
- [x] tests move with their modules; `pytest -q` 11 passed
- [x] `uv run python -m factory.ui.serve` shows the old runs

## 3. One definition of a role
- [x] `AgentConfig.tools: tuple[str, ...]` with the lists now hardcoded in `ROLE_TOOLS` (factory.ts)
- [x] `Board.start()` writes `state/roles.json` `{role: [tool]}`; `solo.py` writes it too
- [x] `factory.ts` reads roles.json instead of `ROLE_TOOLS`
- Verify: `do.py` `run_tester()` live — the tester has read/bash/…/handoff and nothing else; `pytest -q`

## 4. Docs in Open Knowledge Format
- [x] `AGENTS.md` at the root with the "Project knowledge" section (given by Mikhail, verbatim)
- [x] `docs/index.md` — entry point, one line per directory
- [x] `docs/architecture/` — `index.md`, `overview.md` (roles, board, tools, state files: what today lives in
      module docstrings and prompts/README.md), `diagrams/factory.html`
- [x] `docs/decisions/` — `index.md`, the five finished plans renamed by what they decided
      (`events-model.md`, `imitator-test.md`, `secretary.md`, `split-orchestrator.md`, `workspace.md`), this plan joins them when done
- [x] `docs/examples/` — `index.md`, `events.jsonl`
- [x] `factory/roles/prompts/README.md` → `docs/architecture/prompts.md`, the table stays
